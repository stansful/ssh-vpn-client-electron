...
export class WindowsSystemProxyManager {
  ...
  async apply(request: SystemProxyApplyRequest): Promise<SystemProxyApplyResult> {
    ...
    await refreshWindowsProxy();
    return { applied: true, message: `Windows PAC routing enabled for selected domain/IP rules through ${request.socksHost}:${request.socksPort} at ${pacUrl}.` };
  }
...
export function buildProxyPac(rules: RoutingRule[], socksHost: string, socksPort: number): string {
  ...
  const checks = [...domainChecks, ...ipChecks].filter(Boolean);

  return [
    "// Shadow SSH selected-rules PAC. Generated by the desktop client.",
    ...
    "  } catch (e) {}",
    checks.length > 0 ? `  if (${checks.join(" || ")}) { return ${JSON.stringify(proxy)}; }` : "  if (false) { return \"DIRECT\"; }",
    "  return \"DIRECT\";",
    "}",
    ""
  ].join("\n");
